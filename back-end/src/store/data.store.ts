...
export interface User { id:string; firstName:string; lastName:string; email:string; password?:string; role:Role; status:string; skillPoints:number; virtualBalance:number; portfolioValue:number; tradingLimit:number; startingBalance:number; instructorId?:string; studentIds?:string[]; organization?:string; expertise?:string; learnerLevel?:string; }
export interface Stock { symbol:string; name:string; price:number; change:number; changeAbs:number; open:number; high:number; low:number; volume:number; cap:string; sector:string; marketCap:string; vol:string; dataSource:string; lastTickAt?:string; replayIndex?:number; }
export interface Candle { datetime:string; open:number; high:number; low:number; close:number; volume:number; }
export interface Trade { id:string; learnerId:string; symbol:string; type:'BUY'|'SELL'; qty:number; price:number; total:number; date:string; status:string; }

@Injectable()
export class DataStore implements OnModuleInit {
  private logger = new Logger('DataStore');
  users: User[] = [];
  stocks: Stock[] = [];
  candles: Record<string, Candle[]> = {};
  trades: Trade[] = [];
  private marketStartedAt = Date.now();
  private replaySpeedMs = 2000; // 2 seconds in demo = 1 real CSV minute candle
  watchlists: any[] = [];
  courses: any[] = [];
  assignments: any[] = [];
  sessions: any[] = [];
  ...
  feedback: any[] = [];
  diagnostics: any[] = [];
  config: any = { maxDailyTrades: 10, defaultTradingLimit: 150000, maintenanceMode: false, platformName: 'IndAI' };

  private names: Record<string, {name:string; sector:string; cap:string}> = {
  ...
  id(prefix:string){ return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2,7)}`; }
  private seed(){
    this.users = [
    ...
    this.courseModules = [
      {id:'m1', courseId:'c1', title:'Market Basics', type:'video', duration:'15 min', order:1, description:'Introductory market lesson.'},
  ...
  private loadCsvStocks(){
    ...
    for (const file of files){
      ...
      // Avoid duplicate symbol datasets overriding each other when both uppercase and lowercase CSV names exist.
      if (this.candles[symbol]) continue;
      ...
      if (!this.stocks.some(s=>s.symbol===symbol)) this.stocks.push(this.buildStockSnapshot(symbol, meta, candles, `data/stocks/${file}`));
    }
    // frontend had more symbols than datasets; create real-time-compatible fallback copies from existing dataset
  ...
  private parseCsv(file:string): Candle[]{
  ...
  private fakeMarketCap(price:number){ return `${Math.max(0.08, price*0.0012).toFixed(1)}L Cr`; }
  private fakeVolume(candles:Candle[]){ const v=candles.slice(-390).reduce((s,c)=>s+(c.volume||0),0); return v>1000000 ? `${(v/1000000).toFixed(1)}M` : `${Math.round(v/1000)}K`; }
  private currentIndex(symbol:string){
  ...
  private buildStockSnapshot(symbol:string, meta:{name:string; sector:string; cap:string}, candles:Candle[], dataSource:string, index?:number): Stock {
  ...
  getLiveStock(symbol:string){
    ...
    if (existing) Object.assign(existing, live); else this.stocks.push(live);
    return live;
  }
  getLiveStocks(){
    return this.stocks.map(s=>this.getLiveStock(s.symbol)).sort((a,b)=>a.symbol.localeCompare(b.symbol));
  }
  getLiveCandles(symbol:string, limit=240){
    const sym = symbol.toUpperCase();
  ...
  getStock(symbol:string){ return this.getLiveStock(symbol); }
  getHoldings(userId:string){
    const h: Record<string,{qty:number; spent:number}> = {};
    for(const t of this.trades.filter(t=>t.learnerId===userId)){
      h[t.symbol] ||= {qty:0, spent:0};
      if(t.type==='BUY'){ h[t.symbol].qty += t.qty; h[t.symbol].spent += t.total; }
      else { const avg=h[t.symbol].qty ? h[t.symbol].spent/h[t.symbol].qty : 0; h[t.symbol].qty=Math.max(0,h[t.symbol].qty-t.qty); h[t.symbol].spent=Math.max(0,h[t.symbol].spent-avg*t.qty); }
    }
    return h;
  }
  refreshPortfolio(userId:string){
    const u=this.getUser(userId); if(!u) return null;
  ...
  findUserByEmail(email:string){ return this.users.find(u => u.email.toLowerCase() === String(email || '').toLowerCase()); }
  safeUser(user:any){ if(!user) return null; const { password, ...safe } = user; return safe; }
  ensureCourseExists(courseId:string){ return this.courses.find(c => c.id === courseId); }
  ensureLearnerExists(learnerId:string){ return this.users.find(u => u.id === learnerId && u.role === 'learner'); }
