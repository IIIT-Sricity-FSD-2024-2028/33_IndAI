...
const Auth = {
  login(email, password, role) {
    ...
    user.status = status;
    const session = { userId: user.id, role: normalizedRole, email: user.email, name: `${user.firstName || ''} ${user.lastName || ''}`.trim(), user };
  ...
  getCurrentUser() {
    ...
    const found = IndAIData.users.find(u => u.id === s.userId || (sessionUser?.email && String(u.email || '').toLowerCase() === String(sessionUser.email || '').toLowerCase()));
    if (found && sessionUser) {
      // Keep backend-created Swagger users complete across page loads.
      // Local dashboard modules read from IndAIData synchronously, so merge the session profile as a safe fallback.
      Object.assign(found, { ...sessionUser, ...found, password: found.password || sessionUser.password });
      return found;
    }
    if (found) return found;
    if (sessionUser) {
      IndAIData.users = IndAIData.users || [];
      IndAIData.users.push(sessionUser);
...
const DB = {
  ...
  addVideoMaterial({ providerId, courseId, afterModuleId = '', title, description, duration, sizeMB, sourceName, videoUrl, moduleId: providedModuleId, videoId: providedVideoId, status = 'published' }) {
    ...
    const videoId = providedVideoId || genId('v');
    const moduleId = providedModuleId || genId('m');
