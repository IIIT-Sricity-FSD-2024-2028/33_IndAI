      ...
      list.innerHTML = videos.length ? videos.map(v => `
        <div class="course-card" style="overflow:hidden;">
          ...
          <div style="padding:16px;">
            ...
            <div style="display:flex;gap:10px;">
              ${v.videoUrl ? `<button class="btn btn-primary btn-sm" onclick="previewProviderVideo('${v.id}')">Preview</button>` : ''}
    ...
    function handleVideoUpload(file, placement = {}) {
      ...
      if (!allowed.includes(file.type) && !/\.(mp4|webm|mov|avi|mkv)$/i.test(file.name)) {
        UI.showToast("Upload MP4, WEBM, MOV, AVI, or MKV files only.", "error");
        return;
      }
      ...
      probe.onloadedmetadata = async () => {
        const duration = formatVideoDuration(probe.duration);
        const title = placement.title || file.name.replace(/\.[^.]+$/, '').replace(/[-_]/g, ' ');
        const description = placement.description || 'Recently uploaded lesson video.';
        let backendPayload = null;
        try {
          if (window.ApiClient?.uploadCourseVideo) {
            const form = new FormData();
            form.append('courseId', placement.courseId);
            form.append('afterModuleId', placement.afterModuleId || '');
            form.append('title', title);
            form.append('description', description);
            form.append('duration', duration);
            form.append('providerId', providerId);
            form.append('video', file);
            const response = await ApiClient.uploadCourseVideo(form);
            if (response?.success === false) {
              UI.showToast(response.message || 'Backend video upload failed.', 'error');
              URL.revokeObjectURL(objectUrl);
              pendingVideoFile = null;
              return;
            }
            backendPayload = response?.data || response;
          }
        } catch (err) {
          console.warn('Backend video upload failed, saving local preview only:', err.message);
        }

        const moduleData = backendPayload?.module || {};
        const videoData = backendPayload?.video || {};
        const result = DB.addVideoMaterial({
          ...
          videoUrl: moduleData.videoUrl || videoData.videoUrl || objectUrl,
          moduleId: moduleData.id,
          videoId: videoData.id,
          status: moduleData.status || videoData.status || 'published'
        });
        if (backendPayload) URL.revokeObjectURL(objectUrl);
        pendingVideoFile = null;
        if (!result?.success) { UI.showToast(result?.message || 'Upload failed.', 'error'); return; }
        UI.closeModal();
        UI.showToast("Video uploaded successfully! Learners can now watch it in the course.");
        renderVideos();
        renderCourses();
      };
      probe.onerror = () => {
    ...
    function previewProviderVideo(id) {
      const video = (IndAIData.videoLibrary || []).find(v => v.id === id);
      if (!video?.videoUrl) { UI.showToast('No playable video URL found for this item.', 'error'); return; }
      UI.showModal(`
        <div class="modal-title">${video.title}</div>
        <p style="font-size:13px;color:var(--gray-500);margin-bottom:12px;">${video.description || 'Video lesson preview'}</p>
        <video src="${video.videoUrl}" controls style="width:100%;max-height:420px;border-radius:12px;background:#000;"></video>
        <div class="modal-footer"><button class="btn btn-primary" onclick="UI.closeModal()">Close</button></div>`);
    }
